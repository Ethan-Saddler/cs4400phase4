...
def add_airplane():
    ...
    maintenanced_bool = True if maintenanced == 'on' else False
    neo_bool = True if neo == 'on' else False

    conn = get_db_connection()
    cur = conn.cursor()

    try:
        cur.callproc('add_airplane', [airlineID, tail_num, int(seat_capacity), int(speed), locationID,plane_type, maintenanced_bool, model, neo_bool])
        conn.commit()
...
def show_view_data(view_name):
    try:
        ...
        return f"Error fetching data from view {view_name}: {e}"

    html = "<h1>View: {}</h1>".format(view_name)
...
def debug_airplane():
    ...
    if not rows:
        return f"No airplane found with airlineID = {airlineID}, tail_num = {tail_num}"
    return f"Airplane exists: {rows}"

@app.route('/debug/sql')
def debug_sql():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM airline WHERE airlineID = 'Paris'")
    rows = cur.fetchall()
    cur.close()
    conn.close()
